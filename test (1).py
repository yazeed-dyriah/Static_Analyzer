...
def do_the_attrbute(fileName):
    with open(fileName) as file:
        ...
        for line in lines:
            if line.startswith("def"):
                ...
                if not ('int' in arguments[0]
                        and 'str' in arguments[1]
                        and 'str' in arguments[2]):
                    print('incorrect arguments order in this argument :', line)
    ...
    for line in lines:
        if line.replace('\n', '').endswith(')'):
            ...
            if not (sub_line[0].isdecimal() and '"' in sub_line[1] and "'" in sub_line[2]):
...
def main():
    ...
    do_the_attrbute("testFunction.txt")
